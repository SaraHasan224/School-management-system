...
<div class="wrapper">
  ...
  <!-- Content Wrapper. Contains page content -->
    ...
    <section class="content-header">
      <h1>
        <?php $LangList = $this->db->query('SELECT English,Urdu FROM language WHERE English = "Insert School"')->row(); if(!empty($LangList)){ echo $LangList->$Word; }else{echo "Insert School";}?>
      </h1>
    </section>
    <!-- Main content -->
    <section class="content">
      ...
      <div class="box box-default">
        <div class="box-header with-border">
          <button type="button" class="btn btn-default" data-toggle="modal" data-target="#InsertSchool"><?php $LangList = $this->db->query('SELECT English,Urdu FROM language WHERE English = "Insert School"')->row(); if(!empty($LangList)){ echo $LangList->$Word; }else{echo "Insert School";}?></button>
  ...
  <!-- Models -->
  <!-- Modal For Delete -->
  <div class="modal fade" id="InsertSchool">
                <div class="modal-dialog">
                  <div class="modal-content">
                    <div class="modal-header">
                      <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span></button>
                      <h4 class="modal-title"><?php $LangList = $this->db->query('SELECT English,Urdu FROM language WHERE English = "Insert School"')->row(); if(!empty($LangList)){ echo $LangList->$Word; }else{echo "Insert School";}?></h4>
                    </div>
                    <div class="modal-body">
                        <form>
                          ...
                          <div class="col-md-12">
                          <div class="img-top">
                          <img class="img-responsive" id="blah" alt="" src="<?php echo base_url('assets/dist/images/School.jpg');?>" height="150" width="150" />
                                  <div class=''>
                                      ...
                                      <input type="file" name="SchoolLogo" id="imgInp"  class="form-control custom-input-form-control" >
                                      </label>
                                  </div> <!-- text-right / end -->
                            </div>
                          </div>
                          ...
                          <!-- -->
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" id="SchoolName" placeholder="<?php $LangList = $this->db->query('SELECT English,Urdu FROM language WHERE English = "School Name"')->row(); if(!empty($LangList)){ echo $LangList->$Word; }else{echo "School Name";}?>">
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" id="SchoolAddress" placeholder="<?php $LangList = $this->db->query('SELECT English,Urdu FROM language WHERE English = "School Address"')->row(); if(!empty($LangList)){ echo $LangList->$Word; }else{echo "School Address";}?>">
                                </div>
                            </div>
                    ...
                    <div class="modal-footer">
                      ...
                      <button type="button" class="btn btn-primary" onclick="InsertSchool()" data-dismiss="modal"><?php $LangList = $this->db->query('SELECT English,Urdu FROM language WHERE English = "Insert"')->row(); if(!empty($LangList)){ echo $LangList->$Word; }else{echo "Insert";}?></button>
                    </div>
                  </div>
                  <!-- /.modal-content -->
                </div>
...
<script>
  ...
  function InsertSchool() {
        ...
        var SchoolLogo = document.getElementById('imgInp').files[0];
            if (SchoolName != "" && SchoolAddress != "" && SchoolType != "" && SchoolLogo != "") {/******** Check If Fields exits Is Same *********/
                ...
                $.ajax({
                url:"<?php echo base_url('Admin/InsertSchool/Insert'); ?>",
                method:"POST",
                dataType: 'JSON',
                data: form_data,
                contentType: false,
                ...
                success:function(InsertSchool)
                {
                    if (InsertSchool.status == true) {
                        Snackbar.show({pos: 'top-right',text:InsertSchool.message});
                          $.ajax({
                            url:"<?php echo base_url('Admin/InsertSchool'); ?>",
                            beforeSend: function(){
                              $("#page-loader").show();
                            },
                            complete: function(){
  ...
  function DeleteOk() {
                      ...
                      $.ajax({
                        url:"<?php echo base_url('Admin/InsertSchool/Delete'); ?>",
                        method:"POST",
                        dataType: 'JSON',
                        data: form_data,
                        contentType: false,
  ...
  function BlockSchool(Id) {
                      ...
                      $.ajax({
                        url:"<?php echo base_url('Admin/InsertSchool/Block'); ?>",
                        method:"POST",
                        dataType: 'JSON',
                        data: form_data,
                        contentType: false,
                  ...
                  function EditSchool(Id) {
                    ...
                    var edit_data = new FormData();
                      ...
                      $.ajax({
                        url:"<?php echo base_url('Admin/InsertSchool/View'); ?>",
                        method:"POST",
                        dataType: 'JSON',
                        data: edit_data,
                        contentType: false,
          ...
          /**************** Insert Department Field Using Ajax *************/
              function UpdateSchool(Id) {
                ...
                if (SchoolName != "" && SchoolAddress != "" && SchoolType != "") {
                    $.ajax({
                  url:"<?php echo base_url('Admin/InsertSchool/Edit'); ?>",
                  method:"POST",
                  dataType: 'JSON',
                  data: form_data,
                  contentType: false,
