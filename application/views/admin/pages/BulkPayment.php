...
<div class="wrapper">
  ...
  <!-- Content Wrapper. Contains page content -->
    ...
    <section class="content-header">
      <h1>
        <?php $LangList = $this->db->query('SELECT English,Urdu FROM language WHERE English = "Bulk Payment List"')->row(); if(!empty($LangList)){ echo $LangList->$Word; }else{echo "Bulk Payment List";}?>
      </h1>
      ...
      <div class="box box-default">
        ...
        <div class="box-body">
          <div class="row">
            <div class="box-body">
              <div class="row">
              <form method="post" action="<?php echo base_url('BulkPayment/ClassSearch'); ?>">
                <div class="col-md-12">
                  <h3>Class Search</h3>
                </div>
                <div class="col-md-3">
              ...
              <div class="row" style="margin-top:20px;">
                <form method="post" action="<?php echo base_url('BulkPayment/MonthlySearch'); ?>">
                    <div class="col-md-12">
              ...
              <form method="post" action="<?php echo base_url('BulkPayment/Insert'); ?>">
                    <input type="hidden" name="SearchField" value="<?php if(!empty($SearchField)){ echo $SearchField; } ?>">
                    <?php if(!empty($SearchField)){ if($SearchField == 'ClassSearch'){ ?>
                      <input type="hidden" name="ClassId" value="<?php if(!empty($Class)){ echo $Class; } ?>">
                      <input type="hidden" name="Month" value="<?php if(!empty($MonthClass)){ echo $MonthClass; } ?>">
                      <input type="hidden" name="Year" value="<?php if(!empty($YearClass)){ echo $YearClass; } ?>">
                    <?php }elseif($SearchField == 'MonthlySearch'){?> 
              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <th class="text-center"><?php $LangList = $this->db->query('SELECT English,Urdu FROM language WHERE English = "EnterAmount"')->row(); if(!empty($LangList)){ echo $LangList->$Word; }else{echo "EnterAmount";}?></th>
                ...
                <tbody id="feedetails">
                  <?php $TotalDuesCalculated=0; $i = 1; if(!empty($StudentList)){ foreach ($StudentList as $STULIS) {
                    $Dues = $this->Admindb->SumRecord(['StudentId'=>$STULIS->StudentId,'MonthNumber <='=>$STULIS->MonthNumber,'IsActive'=>true,'IsDeleted'=>false],'Dues','fee');
                    ?>
                <tr>
                  <td class="text-center"><div class="pl-3 border-bottom cxm-children">
                  ...
                  <td class="text-center"><?php echo $DuesCalculated ?></td>
                  <td class="text-center"><?php echo $this->Admindb->SumRecord(['StudentId'=>$STULIS->StudentId,'FeeMonth'=>$STULIS->FeeMonth,'IsActive'=>true,'IsDeleted'=>false],'AmountPaid','fee'); ?></td>
                  <td class="text-center"><?php if($Dues <= 0){ echo "Paid"; }else{ ?><input type="text" class="form-control" name="Amount[]" placeholder="Enter Amount"> <input type="hidden" name="StudentId[]" value="<?php echo $STULIS->StudentId; ?>"><input type="hidden" name="Fee[]" value="<?php echo $STULIS->Fee; ?>"><?php } ?></td>
                  <td class="text-center"><?php if($Dues <= 0){ echo $STULIS->PaidDate; }else{ ?><input type="text" class="form-control" name="PaidDate[]" id="datepicker<?php echo $i; ?>" placeholder="Paid Date"> <?php } ?></td>
                  <td class="text-center"><?php if($Dues <= 0){ echo $STULIS->BankRef; }else{ ?><input type="text" class="form-control" name="BankRef[]" placeholder="Bank Ref"> <?php } ?></td>
                </tr>
              ...
              function filterdata(){
                ...
                var form_data = new FormData();
                      ...
                      form_data.append("Year", Year);
                          $.ajax({
                        url:"<?php echo base_url('Admin/BulkPayment/Filter'); ?>",
                        method:"POST",
                        dataType: 'JSON',
                        data: form_data,
                        contentType: false,
