...
<body>
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      <a href="<?php echo base_url('PromoteAll'); ?>" class="btn btn-warning">Promote All</a>
      <p class="<?php if($this->session->flashdata('CheckStatus') == 'True'){ echo "text-success"; }else{ echo "text-danger"; } ?>"><?php if($this->session->flashdata('InsertSubject')){ 
              echo $this->session->flashdata('Promoted'); }
              ?></p>
      ...
      <?php if(!empty($ClassList)){ foreach($ClassList as $CLALI){ ?>
        ...
        <div class="col-lg-3 col-xs-6">
          <!-- small box -->
          <a href="<?php echo base_url('ClassStudentResult/'.$CLALI->ClassId) ?>">
